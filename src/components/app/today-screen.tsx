  ...
  if (!session) {
    return (
      <div className="mx-auto max-w-lg px-4 py-16 text-center">
        ...
        <p className="mt-2 text-sm text-zinc-400">Nine short questions so Today can show your exposure and financial break point.</p>
  ...
  return (
    <div className="mx-auto max-w-2xl space-y-5 px-4 py-6 lg:px-8">
      <div>
        ...
        <div className="mt-2 flex flex-wrap items-center gap-2 text-xs text-zinc-500">
          ...
          {premium ? (
            ...
            <span className="rounded-full border border-white/10 bg-white/[0.04] px-2.5 py-1 text-zinc-500">Free plan</span>
          )}
        </div>
      </div>
      ...
      <div className="grid gap-3 sm:grid-cols-[1fr_1.1fr]">
        ...
        <GlassCard tone={((premium ? exposure?.primary.days : financial?.days) ?? 99) < 30 ? "danger" : "default"}>
          ...
          <p className="mt-2 text-2xl font-bold tabular-nums text-zinc-50">
            {premium
              ...
              : (financial?.value || exposure?.primary.value || `${runwayDays} days`)}
          </p>
          <p className="mt-2 text-sm leading-relaxed text-zinc-400">
            {premium
              ? (exposure?.primary.meaning || `If primary income stops, reserves cover about ${runwayDays} days.`)
              : (financial?.meaning || exposure?.primary.meaning || `If primary income stops, reserves cover about ${runwayDays} days.`)}
          </p>
          {premium ? (
            ...
            <Link href="/#pricing" className="mt-3 inline-block text-xs font-semibold text-emerald-400">
          ...
          {exposure.points.slice(1, 4).map((bp) => {
            ...
            return (
              ...
              >
                ...
                {locked ? (
                  <>
                    <p className="mt-1 select-none text-lg font-bold tabular-nums text-zinc-600 blur-[2px]">••</p>
                    <span className="absolute right-2 top-2 rounded-full bg-emerald-500/15 px-1.5 py-0.5 text-[9px] font-semibold text-emerald-400">Pro</span>
                  </>
                ) : (
                  <p
                    className={
      ...
      {premium ? (
        ...
        <UpgradeGate variant="inline" title="Full action plan and 12-month tracker need Pro" />
      )}
      ...
      {!premium && (
        <GlassCard tone="accent">
          ...
          <p className="mt-1 text-xs text-zinc-500">Pro · $15/mo — all four break points, What If?, vault, year tracker.</p>
          <Button size="sm" className="mt-3" disabled={paying} onClick={() => void unlock()}>{paying ? "Opening…" : "Unlock Pro · $15/mo"}</Button>
        </GlassCard>
      )}
    </div>
  );
