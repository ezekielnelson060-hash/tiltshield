...
export default function WhatIfPage() {
  ...
  function run(id: WhatIfScenario, free?: boolean) {
    ...
    if (!free && !premium) {
      if (
        window.confirm(
          "Full scenarios unlock with the founding plan. Unlock for demo?"
        )
      ) {
        setPremium(true);
        setPrem(true);
    ...
    <div className="mx-auto max-w-2xl space-y-8 px-4 py-8 lg:px-8">
      <div>
        ...
        <p className="mt-1 text-sm text-zinc-500">
          Built from your answers — not generic advice.
        </p>
        <p className="mt-3 text-xs text-zinc-600">
          Your baseline: ~{runway} day cash runway · ${
          (a.monthly_expenses || 0).toLocaleString()}/mo essentials ·{" "}
